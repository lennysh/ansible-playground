# demos/jinja2-filters — Jinja2 built-in filter reference
...
Ansible also ships **ansible.builtin** filters (see [demos/ansible-filters](../ansible-filters/README.md)); those are separate from the stock Jinja2 filters covered here.
...
Several names exist in both worlds (for example `default`, `join`, `unique`). In Ansible playbooks, **ansible.builtin filters take precedence** when names collide. This demo uses expressions that exercise the Jinja2 implementations; for Ansible-specific behavior see [demos/ansible-filters](../ansible-filters/README.md).
...
- Pipe survey answers through `| bool` when you need real booleans — Jinja2 truthiness treats the string `"false"` as true (see [demos/when](../when/README.md)).

## How to run

```bash
cd demos/jinja2-filters
