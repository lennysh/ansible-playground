# demos/winrm-vs-psrp — WinRM vs PSRP timing with Kerberos
...
At the end, a formatted report is written to **`connection-benchmark-report.txt`** in the demo directory (same pattern as `demos/kerberos-winrm`):

```bash
less demos/winrm-vs-psrp/connection-benchmark-report.txt
```
